...
#4)  Run LPMerge across multiple intervals on each linkage group.
#5)  Select the best interval for each linkage group based on RMSE and linkage group length. 
#6)  Test map synteny with the component species maps. 
...
#create input of only markers that are found on both maps
...
##STEP 4: Run in LPMerge to integrate maps by linkage group
...
#reload to determine "best" interval for each chromosome
...
#save thsi file as the finalized overlap output
write.csv(ovout_best, file=paste(outputdir, "overlap_markers_weighted_bestchrs.csv", sep="/"))

#visually compare overlap map to each species component map
library(tidyr)
...
#format this dataframe for input into LPMerge 
...
##Create map using all possible markers by combining each species map with the overlap map
...
#manually remove gaps in excel using graphs from above to inform
...
##STEP 6: Compare to Sitka and White spruce maps
#Load 3 maps 
