...
write.csv(subset, file=paste(output, "RADSNP_SCLM2_Cook4NARC_Map.csv", sep=""))

#add in SNP IDs
ID <- read.table(file=paste(datafiles, "RADSNPC4N_SNPIDs.txt", sep=""))
colnames(ID) <- c("CHR","POS","SNPID")
CallID <- read.table(file=paste(datafiles, "RADSNPC4N_callfile.txt", sep=""), -1)
...
write.csv(withSNPID, file=paste(output , "RADSNP_SCLM2_Cook4NARC_Map_SNPID.csv", sep=""))
