...
write.csv(subset, file=paste(output, "RADSNP_LM1_Map.csv", sep=""))

#add in SNP IDs
ID <- read.table(file=paste(datafiles, "RADSNP_LM1_SNPIDs.txt", sep=""))
colnames(ID) <- c("CHR","POS","SNPID")
CallID <- read.table(file=paste(output, "RADSNP_LM1_snps_callfile.txt", sep=""), -1)
...
write.csv(withSNPID, file=paste(output, "RADSNP_LM1_Map_SNPID.csv", sep=""))
