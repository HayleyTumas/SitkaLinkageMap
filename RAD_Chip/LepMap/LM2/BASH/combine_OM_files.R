...
subset <- full[,c(1:2, 10)]
colnames(subset) <- c("SNP","Position","LG")
write.csv(subset, file=paste(output, "RADSNP_LM2_Map.csv", sep=""))

#add in SNP IDs
ID <- read.table(file=paste(datafiles, "RADSNP_LM2_SNPIDs.txt", sep=""))
colnames(ID) <- c("CHR","POS","SNPID")
CallID <- read.table(file=paste(datafiles, "RADSNP_LM2_snps_callfile.txt", sep=""), -1)
...
write.csv(withSNPID, file=paste(output, "LM2/RADSNP_LM2_Map_SNPID.csv", sep=""))
